...
| Fraud network graph intelligence | Implemented with live-ingest path | GAT node risk, distribution-shift/score-collapse safeguard, evidence anomaly fallback, community detection, operational entity/edge/event ingestion, and provenance-aware evidence output. Built-in demo fallback currently has 69 nodes/106 edges; sandbox bootstrap seeds 60 operational entities/90 operational edges. |
